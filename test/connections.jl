...
@test Simulacrum.connect!(CHG(intvars, CHE{Int64}[]), intvars, CHE, Cycle)
@test Simulacrum.connect!(CHG(intvars, CHE{Int64}[]), intvars, CHE, Cycle, to_hyperedge_idx = false)
@test Simulacrum.connect!(CHG(symvars, CHE{Num}[]), symvars, CHE, Cycle)
@test Simulacrum.connect!(CHG(symvars, CHE{Num}[]), symvars, CHE, Cycle, to_hyperedge_idx = false)
chg = CHG(symvars, CHE{Num}[])
@test Simulacrum.connect!(chg, symvars, CHE, AllToAll)
@test nhe(chg) == 3
chg = CHG(symvars, CHE{Num}[])
@test Simulacrum.connect!(chg, symvars, CHE, Cycle)
@test nhe(chg) == 3
chg = CHG(symvars, CHE{Num}[])
@test Simulacrum.connect!(chg, symvars, CHE, Path)
@test nhe(chg) == 2

template_che = CHE(SpeciesSet(10, 2), SpeciesSet(20, 1), 3)
chg = CHG(intvars, CHE{Int64}[])
@test Simulacrum.connect!(chg, intvars, template_che, Cycle)
@test all(weight.(hyperedges(chg)) .== weight(template_che) .== 3)
chg = CHG(intvars, CHE{Int64}[])
@test Simulacrum.connect!(chg, intvars, template_che, AllToAll, to_hyperedge_idx = false)
@test all(weight.(hyperedges(chg)) .== weight(template_che) .== 3)
@test nhe(chg) == 6
template_che = CHE(SpeciesSet(X, 2), SpeciesSet(X, 1), 3)
chg = CHG(symvars, CHE{Num}[])
@test Simulacrum.connect!(chg, symvars, template_che, Cycle)
@test all(weight.(hyperedges(chg)) .== weight(template_che) .== 3)
chg = CHG(symvars, CHE{Num}[])
@test Simulacrum.connect!(chg, symvars, template_che, AllToAll, to_hyperedge_idx = false)
@test all(weight.(hyperedges(chg)) .== weight(template_che) .== 3)
@test nhe(chg) == 6
